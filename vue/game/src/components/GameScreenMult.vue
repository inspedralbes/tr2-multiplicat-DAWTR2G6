...
export default {
  ...
  methods: {
    ...
    evento_respuestaEnviada(opcion) {
      ...
      if (respuestaUsuarioTrimmed === respuestaCorrecta) {
        ...
        this.blocks = Math.max(this.blocks + 1, 0); //Asegurem que no sigui negatiu
      }
    ...
    getPreguntaActual() {
      return {
        enunciado:
          this.data_preguntas["preguntas_unidades"][this.index].pregunta,
        opciones: JSON.parse(
          this.data_preguntas["preguntas_unidades"][this.index].opciones
        ),
        respuesta_correcta:
          this.data_preguntas["preguntas_unidades"][this.index]
            .respuesta_correcta,
      };
    },
    guardarRespuestasParaProfesor(opcion) {
      ...
      this.partida_respuestas.push(
        this.preguntaActual.respuesta_correcta.trim()
      );
    },
    partidaAcabada() {
  ...
  created() {
    ...
    socket.on("mover_sala_a_scores", (room) => {
      console.log(`Redirigiendo a la sala ${room} a la pantalla de scores`);
      this.$router.push({ name: "ScoreScreen", params: { id: socket.id } });
    });
  },
  beforeDestroy() {
